// ============================================================
// MTM VISUALIZADOR - PLAYOFFS BRACKET PATCH v2.1
// ============================================================
...
// ============================================================
// FIX v2.1:
//   - Transición suave al cambiar de ronda (sin flash/parpadeo).
//   - Menú "Stats Playoffs" ahora se inyecta correctamente.
//   - Escudos se incluyen en "Imprimir Bracket".
// ============================================================
...
(function () {
    ...
    function selectGame(idx) {
        ...
        // Solo actualizar el cuerpo de stats (con animación)
        const body = document.getElementById('pbx-modal-stats-body');
        if (body) {
            body.innerHTML = buildStatsContentHtml();
            // La animación CSS se dispara automáticamente al re-crear .pbx-stats-label y .match-stats-body
        }
    }
    ...
    function injectPlayoffStatsMenu() {
        ...
        // Buscar el nav-item de playoffs por data-section (NO por id, que no existe)
        const playoffsNav = document.querySelector('.nav-item[data-section="playoffs"]');
        if (!playoffsNav) {
            // Reintentar en 200ms (buildUI puede no haber terminado)
            setTimeout(injectPlayoffStatsMenu, 200);
            return;
        }

        // Si ya existe, actualizar el section pero no duplicar nav
...
`;
    ...
    window.buildUI = function (torneo) {
        ...
        // Esperar a que el DOM se asiente y luego inyectar
        setTimeout(() => {
    ...
    // Si ya hay un torneo cargado al momento de cargar el parche (poco probable, pero por si acaso)
    ...
    console.log('✅ Playoffs Bracket Patch v2.1 cargado (fixes: transición suave, menú stats, escudos en print).');
})();