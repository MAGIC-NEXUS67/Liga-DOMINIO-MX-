// ============================================================
// MTM VISUALIZADOR - PLAYER HISTORY PATCH v1.1
// ============================================================
// FIX v1.1:
//   - ID correcto: profile-modal-content (antes player-profile-content)
//   - Acceso correcto a bindings globales (currentData, data1, data2,
//     MENU_ITEMS, perfSelectedPlayerId) — no están en window
//   - Reintentos de inyección por si el DOM tarda
...
(function () {
    ...
    // ============================================================
    // 0. GUARDS Y HELPER PARA BINDINGS GLOBALES
    // ============================================================
    // En script.js hay `let currentData = null;` (global léxico).
    // NO es window.currentData, pero ES accesible directamente.
    // Usamos una función que evita ReferenceError si por alguna razón
    // el script.js aún no se ejecutó.
    function g(name) {
        try {
    ...
    window.selectTournament = function (division) {
        if (division === 3) {
            ...
            // Asignar variables globales de script.js
            // currentData es un let global (no está en window), hay que usar eval-ish
            try {
                // eslint-disable-next-line no-eval
    ...
    function openMatchFromHistory(tournamentId, matchId, matchType) {
        ...
        // Poner temporalmente como currentData via eval
        try {
            // eslint-disable-next-line no-eval
    ...
    window.showMatchStats = function (matchId) {
        ...
        const cd = getCurrentData();
        if (cd) {
            if ((cd.rounds || []).some(r => (r || []).some(m => m.id === matchId))) inCurrentData = true;
            if (!inCurrentData && cd.friendlyMatches && cd.friendlyMatches.some(m => m.id === matchId)) inCurrentData = true;
        }
    ...
    // ============================================================
    // 9. OVERRIDE: showPlayerProfile
    //    ¡ID CORREGIDO! -> profile-modal-content
    // ============================================================
    ...
    window.showPlayerProfile = function (playerId) {
        ...
        // Reintentar por si el DOM tarda (aunque el original es síncrono)
    ...
    function patchPlayerProfile(playerId) {
        ...
        try {
            ...
            if (statsGrid) {
                ...
                console.warn('[player-history] .profile-stats-grid no encontrado en el perfil');
            }
        } catch (e) {
        ...
        if (actionsLeft && !actionsLeft.querySelector('.btn-history')) {
            ...
            console.warn('[player-history] .profile-actions-left no encontrado');
        }
    ...
    // ============================================================
    // 10. OVERRIDE: renderPerformanceJugadores
    // ============================================================
...
@media (max-width: 700px) {
    ...
    .pbh-item {
        grid-template-columns: 1fr; gap: 0.4rem; padding: 0.6rem;
    }
...
`;
    ...
    console.log('✅ Player History Patch v1.1 cargado (fix ID: profile-modal-content).');
})();