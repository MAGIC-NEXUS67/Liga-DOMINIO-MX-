...
// script.js v3 - Sistema de impresión avanzado + comparaciones
// ==========================================
...
// NUEVO: Resolver variables CSS a hex para impresión
...
// NUEVO: Helper para abrir modal de configuración de impresión
function openPrintConfigModal(title, bodyHtml, onConfirm) {
  const existing = document.getElementById('print-config-overlay');
  if (existing) existing.remove();

  const overlay = document.createElement('div');
  overlay.id = 'print-config-overlay';
  overlay.className = 'print-config-overlay';
  overlay.innerHTML = `
    <div class="print-config-modal">
      <h3>${title}</h3>
      <div class="print-config-body">${bodyHtml}</div>
      <div class="print-config-actions">
        <button class="btn-cancel-config">Cancelar</button>
        <button class="btn-confirm-config">🖨️ Imprimir</button>
      </div>
    </div>
  `;
  document.body.appendChild(overlay);

  overlay.querySelector('.btn-cancel-config').addEventListener('click', () => overlay.remove());
...
// NUEVO: Helper para obtener jugadores ordenados por key
...
// NUEVO: Cálculo de ganador (jugadores)
...
// NUEVO: Cálculo de ganador (equipos)
...
// NUEVO: Podio mejorado para impresión
function renderPrintPodium(top3) {
  if (!top3 || top3.length === 0) return '<div style="text-align:center;color:#94a3b8;padding:2rem;">Sin datos</div>';
  const colors = ['#ffd700', '#c0c0c0', '#cd7f32'];
  const sizes = [110, 90, 78];
  const order = [1, 0, 2];
  let html = '<div style="display:flex;justify-content:center;align-items:flex-end;gap:1.8rem;padding:2.5rem 0 1.5rem;min-height:280px;">';
  order.forEach((idx) => {
    const p = top3[idx];
    if (!p) return;
    const pos = idx + 1;
    ...
    const crown = idx === 0 ? '<div style="position:absolute;top:-30px;font-size:1.8rem;filter:drop-shadow(0 0 8px rgba(255,215,0,0.7));">👑</div>' : '';
    const offsetY = idx === 0 ? 'translateY(-25px)' : (idx === 2 ? 'translateY(15px)' : 'translateY(0)');
    html += `
      <div style="display:flex;flex-direction:column;align-items:center;text-align:center;position:relative;transform:${offsetY};">
        ${crown}
        <div style="position:relative;">
          <div style="width:${size}px;height:${size}px;border-radius:50%;background:#000;border:3px solid ${color};overflow:hidden;display:flex;align-items:center;justify-content:center;box-shadow:0 6px 24px rgba(0,0,0,0.6), 0 0 20px ${color}40;">
            <img src="${p.teamShield || ''}" style="width:100%;height:100%;object-fit:contain;background:#000;">
          </div>
          <div style="position:absolute;bottom:-8px;left:50%;transform:translateX(-50%);background:${color};color:#000;font-weight:900;font-size:0.75rem;width:26px;height:26px;border-radius:50%;display:flex;align-items:center;justify-content:center;border:2px solid #0a0a0f;">#${pos}</div>
        </div>
        <div style="font-weight:700;font-size:0.9rem;color:#fff;max-width:150px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;margin-top:0.7rem;">${p.name}</div>
        <div style="font-weight:900;font-size:1.25rem;color:#ffd700;text-shadow:0 0 12px rgba(255,215,0,0.5);margin-top:0.15rem;">${(p.pig||0).toFixed(1)} PIG</div>
        <div style="font-size:0.68rem;color:#94a3b8;margin-top:0.15rem;">${p.teamName}</div>
      </div>
    `;
  });
  html += '</div>';
...
// NUEVO: Build tabla de stats para impresión
...
// NUEVO: HTML para comparación de jugadores
...
// NUEVO: HTML para comparación de equipos
...
function getPrintHeader(title) {
  ...
  return `
    <div style="display:flex;align-items:center;gap:1rem;margin-bottom:0.5rem;border-bottom:2px solid #2d2d44;padding-bottom:0.5rem;">
      ...
      <div>
        <div style="font-family:'Montserrat',sans-serif;font-weight:900;font-size:1.2rem;color:#a855f7;">${leagueName}</div>
...
// ==========================================
// DIBUJO DE GRÁFICO (con chartZoom)
// ==========================================
...
// ==========================================
// TOOLTIP (Hit-testing circular preciso)
// ==========================================
...
// ==========================================
// NUEVA printPerformance() con stats + peaks
// ==========================================
...
// ==========================================
// NUEVA printStats() con modal
// ==========================================
function printStats() {
  const torneo = currentData;
  if (!torneo) return alert('No hay torneo activo.');
...
// ==========================================
// NUEVA printBalon() con modal y podio mejorado
// ==========================================
function printBalon() {
  const torneo = currentData;
  if (!torneo) return alert('No hay datos.');
...
// ==========================================
// NUEVA printMercado() con modal + comparación
// ==========================================
...
function printEquipos() {
  const container = document.querySelector('#section-equipos');
  if (!container) return alert('No hay equipos para imprimir.');
  const clone = container.cloneNode(true);
  const titleEl = clone.querySelector('.section-title');
  if (titleEl) titleEl.remove();
  let html = getPrintHeader('EQUIPOS') + clone.innerHTML;
  html = resolveCSSVars(html);
  showPrintModal(html, 'equipos.jpg', 1100);
}
