  ...
  function drawPerformanceChart(canvasId, data, type, labels, visibleLines) {
    ...
    // Ancho = ancho del contenedor × zoom (sin límite superior)
    ...
    // Guardar puntos para el tooltip y el zoom
  ...
  function setupTooltip(canvasId) {
    ...
    // Radio de hit-testing: 8px en PC, 14px en móvil (touch)
    ...
    function getCanvasCoords(e) {
      ...
      // Coordenadas del cursor en px CSS, relativas al canvas
    ...
    function showTooltip(e) {
      ...
      // Hit-testing circular: el punto más cercano por distancia euclidiana
      // que esté dentro del radio HIT_RADIUS
      let closest = null;
      ...
      // Posicionamiento con clamp en los 4 límites para que nunca se salga
      const tooltipW = 300;
      const tooltipH = 240;
  ...
  function printPerformance() {
    ...
    showPrintModal(htmlContent, `performance_${sanitizeForFilename(title)}.jpg`, 850);
  }
  ...
  // ==========================================
  // IMPRESIÓN DE PARTIDO (con logos y stats)
  // ==========================================
  window.printMatchStats = function(matchId) {
    ...
    document.querySelector('.print-modal-overlay:not([style*="display: none"])')?.remove();
    // Remover overlays residuales del modal de stats
    document.querySelectorAll('.print-modal-overlay').forEach((el, i) => {
      if (i < document.querySelectorAll('.print-modal-overlay').length - 1) el.remove();
    });
  };
  ...
  // ==========================================
  // STATS (con scroll interno, sin botón "Ver 10 más")
  // ==========================================
  ...
  function renderEquipos(torneo) {
    ...
    var html = '<div style="display:grid;grid-template-columns:repeat(auto-fill,minmax(340px,1fr));gap:1.5rem;">';
    torneo.teams.forEach(function(team) {
        ...
        var renderPlayerSlots = function(players, cls) {
  ...
  // ==========================================
  // IMPRESIÓN DE SECCIONES (restauradas)
  // ==========================================
  ...
  // ==========================================
  // IMPRESIÓN DE JORNADA (con escudos y estilo)
  // ==========================================
  function printJornada() {
    ...
    window.confirmPrintJornada = function() {
      ...
      var html = getPrintHeader('JORNADA ' + (idx+1));
      ...
      round.forEach(function(m, i) {
        ...
        var scoreColor = played ? '#facc15' : '#94a3b8';
        var rowBg = i % 2 === 0 ? 'rgba(30,30,47,0.7)' : 'rgba(20,20,30,0.7)';
  ...
  function printStats() {
    var container = document.querySelector('#section-stats');
    if (!container) return alert('No hay estadísticas para imprimir.');
    var clone = container.cloneNode(true);
    // Remover el título (ya lo pone el header de impresión)
    var titleEl = clone.querySelector('.section-title');
    if (titleEl) titleEl.remove();
    var html = getPrintHeader('STATS') + clone.innerHTML;
    showPrintModal(html, 'stats.jpg', 900);
  }

  function printBalon() {
    var container = document.querySelector('#section-balon');
    if (!container) return alert('No hay Balón de Oro para imprimir.');
    var clone = container.cloneNode(true);
    var titleEl = clone.querySelector('.section-title');
    if (titleEl) titleEl.remove();
    var html = getPrintHeader('BALÓN DE ORO') + clone.innerHTML;
    showPrintModal(html, 'balon_oro.jpg', 900);
  }

  function printEquipos() {
    var container = document.querySelector('#section-equipos');
    if (!container) return alert('No hay equipos para imprimir.');
    var clone = container.cloneNode(true);
    var titleEl = clone.querySelector('.section-title');
    if (titleEl) titleEl.remove();
    var html = getPrintHeader('EQUIPOS') + clone.innerHTML;
    showPrintModal(html, 'equipos.jpg', 1100);
  }

  function printMercado() {
    var container = document.getElementById('mercadoTablaContainer');
    if (!container || !container.querySelector('table')) return alert('No hay tabla de mercado para imprimir.');
    var clone = container.cloneNode(true);
    var html = getPrintHeader('MERCADO') + clone.outerHTML + '<div style="margin-top:0.3rem;font-size:0.55rem;color:#94a3b8;">' + currentData.name + '</div>';
    showPrintModal(html, 'mercado.jpg', 900);
  }

  function printEconomia() {
    var container = document.querySelector('#section-economia');
    if (!container) return alert('No hay movimientos económicos para imprimir.');
    var clone = container.cloneNode(true);
    var titleEl = clone.querySelector('.section-title');
    if (titleEl) titleEl.remove();
    var html = getPrintHeader('MOVIMIENTOS ECONÓMICOS') + clone.innerHTML;
    showPrintModal(html, 'economia.jpg', 1000);
  }
