...
// script.js v5 - Fixes: scroll general, equipos, jornadas, modal partido, stats, sin power duplicado
// ==========================================
...
let statsShowLimit = { goals: 10, assists: 10, saves: 10, shots: 10 };
...
// ==========================================
// FASE C — JORNADAS estilo moderno
// ==========================================
...
// ==========================================
// FASE D — Modal de Stats de Partido
// ==========================================
...
// ==========================================
// FASE E — Stats Table (moderna) — FASE F: sin Power
// ==========================================
function buildStatsRows(players, key) {
  return players.map((p, i) => {
    const rank = i + 1;
    const rankClass = rank === 1 ? 'rank-1' : (rank === 2 ? 'rank-2' : (rank === 3 ? 'rank-3' : ''));
    const valueClass = rank === 1 ? 'legendary' : (rank <= 3 ? 'elite' : 'standard');
    return `<tr class="stats-row-clickable ${rankClass}" onclick="showPlayerProfile('${p.id}')">
      <td class="rank-cell">${rank}</td>
      <td class="player-cell"><img src="${p.teamShield||''}" class="shield-sm"> ${p.isCaptain ? '👑 ' : ''}${p.name}</td>
      <td style="color:var(--color-text-muted);font-size:0.75rem;">${p.teamName}</td>
      <td class="value-cell ${valueClass}">${p[key]||0}</td>
    </tr>`;
  }).join('');
}

function renderStats(torneo, loadMoreKey = null) {
  if (loadMoreKey) {
    statsShowLimit[loadMoreKey] += 10;
    const container = document.getElementById('stats-' + loadMoreKey);
    if (container) {
      const allPlayers = getSortedPlayersByKey(torneo, loadMoreKey);
      const visible = allPlayers.slice(0, statsShowLimit[loadMoreKey]);
      const hasMore = statsShowLimit[loadMoreKey] < allPlayers.length;
      container.querySelector('tbody').innerHTML = buildStatsRows(visible, loadMoreKey);
      const card = container.closest('.card');
      if (card) {
        const btnOld = card.querySelector('.stats-show-more');
        if (btnOld) btnOld.remove();
        const countLabel = card.querySelector('.stats-count-label');
        if (countLabel) countLabel.textContent = `Top ${statsShowLimit[loadMoreKey]} de ${allPlayers.length}`;
        if (hasMore) {
          const btn = document.createElement('button');
          btn.className = 'stats-show-more';
          btn.innerHTML = '<i class="fas fa-chevron-down" style="margin-right:0.3rem;"></i> Mostrar 10 más...';
          btn.onclick = () => renderStats(torneo, loadMoreKey);
  ...
  function makeTable(title, key, label) {
    ...
    const limit = statsShowLimit[key] || 10;
    ...
    return `
      <div class="card" style="padding:0.85rem;">
        <div class="card-header" style="font-size:0.78rem;">
          ...
          <span class="stats-count-label" style="font-size:0.62rem;color:var(--color-text-muted);font-weight:700;">Top ${limit} de ${sorted.length}</span>
        </div>
        <div class="stats-table-wrap" id="stats-${key}">
          <table>
            <thead>
              <tr>
        ...
        ${hasMore ? `<button class="stats-show-more" onclick="renderStats(currentData, '${key}')"><i class="fas fa-chevron-down" style="margin-right:0.3rem;"></i> Mostrar 10 más...</button>` : ''}
      </div>
    `;
  }
...
// ==========================================
// FASE B — EQUIPOS (un solo scroll por card)
// ==========================================
